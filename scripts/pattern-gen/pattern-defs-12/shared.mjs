...
//
// Batch 12 keeps batch 11's constraint (native SVG export with no caveat) and
// re-exports its vocabulary from pattern-defs-11/shared.mjs, which lists what
// is off the table and why. What this batch adds is the *smooth* gradient,
// and two things make a smooth ramp safe where a conic one is not:
//
...
//     faded end a real hole: with the background slot transparent, the sheet
//     shows through the soft edge exactly as it does through a hard one.
//
// The house rules are listed in pattern-defs-12.mjs.
export {
  RESERVED,
  section,
  F,
...
// Every motif name used anywhere in the project, batch 11 included.
