  ...
  return (
    <div className={s.page}>
      ...
      <header className={s.bar}>
        ...
        <a className={s.barCta} href="#enroll">Enroll</a>
      ...
      <main id="top">
        ...
        <section className={s.hero} aria-labelledby="hero-h">
          <div className={s.heroCopy}>
            <p className={s.kicker}>Summer day camp, ages 5 to 12, on Loon Lake</p>
            ...
            <div className={s.heroActions}>
              <a className={s.btn} href="#sessions">See the eight weeks</a>
              <a className={s.btnLine} href="#enroll">Hold a spot</a>
            </div>
          ...
          <div className={s.heroPanel}>
            ...
            <p className={s.panelNote}>
              <span>Friday fire circle</span>
              <strong>2:00 pm, every week</strong>
            </p>
          </div>
        </section>
        ...
        <section id="sessions" className={s.sessions} aria-labelledby="sessions-h">
          <div className={s.head}>
            <p className={s.eyebrow}>The summer</p>
            <h2 id="sessions-h">Eight weeks, eight themes</h2>
          ...
          <div className={s.timelineWrap}>
            <ol className={s.months} aria-hidden="true">
              <li className={s.monthJun}>June</li>
              <li className={s.monthJul}>July</li>
              <li className={s.monthAug}>August</li>
            </ol>
            <ol className={s.timeline}>
              {WEEKS.map((w) => (
                <li key={w.n} className={`${s.week} ${s[w.state]}`}>
                  ...
                  <time className={s.weekDates}>{w.dates}</time>
                  <h3 className={s.weekTheme}>{w.theme}</h3>
                  <p className={s.weekBody}>{w.body}</p>
                  <span className={s.weekStatus}>{w.status}</span>
                </li>
              ))}
            </ol>
          </div>
          <ul className={s.groups}>
            {GROUPS.map(([name, ages, ratio]) => (
              <li key={name}>
                <strong>{name}</strong>
                <span>{ages}</span>
                <span>{ratio}</span>
        ...
        <section id="activities" className={s.activities} aria-labelledby="activities-h">
          ...
          <div className={s.activitiesInner}>
            <div className={s.head}>
              <p className={s.eyebrow}>Activities</p>
              <h2 id="activities-h">What a week is made of</h2>
              <p className={s.headNote}>
            ...
            <ul className={s.cards}>
              {ACTIVITIES.map((a) => (
                <li key={a.title} className={s.card}>
                  ...
                  <span className={s.cardTag}>{a.tag}</span>
        ...
        <section id="day" className={s.day} aria-labelledby="day-h">
          <div className={s.dayAside}>
            <div className={s.head}>
              <p className={s.eyebrow}>A day at camp</p>
              <h2 id="day-h">From the Big Pine to pick-up</h2>
              <p className={s.headNote}>
            ...
            <div className={s.pack}>
              <h3 className={s.packHead}>In the backpack</h3>
          ...
          <ol className={s.schedule}>
            {DAY.map(([time, what, note]) => (
              <li key={time}>
                <time className={s.slotTime}>{time}</time>
                <strong className={s.slotWhat}>{what}</strong>
                <span className={s.slotNote}>{note}</span>
              </li>
            ))}
          </ol>
        </section>
        ...
        <section id="fees" className={s.fees} aria-labelledby="fees-h">
          <div className={s.feesInner}>
            ...
            <table className={s.feeTable}>
              <caption className={s.srOnly}>Camp fees for the summer</caption>
              <tbody>
                {FEES.map(([what, cost]) => (
                  <tr key={what}>
            ...
            <div className={s.aid}>
              <h3 className={s.aidHead}>One camper in five comes on a scholarship</h3>
              <p className={s.aidBody}>
        ...
        <section id="enroll" className={s.enroll} aria-labelledby="enroll-h">
          <div className={s.enrollIntro}>
            <p className={s.eyebrow}>Enroll</p>
            <h2 id="enroll-h">Hold your camper's weeks</h2>
            <p className={s.headNote}>
          ...
          <form className={s.form} action="#">
            <div className={s.formRow}>
              <label className={s.field}>
                <span>Camper's name</span>
                <input type="text" name="camper" autoComplete="off" />
              </label>
              <label className={s.field}>
                <span>Age this summer</span>
            ...
            <div className={s.formFoot}>
              <label className={s.check}>
                ...
                <span>Add aftercare until 5:30</span>
              </label>
              <button type="submit" className={s.btn}>Hold the weeks</button>
            </div>
          </form>
        </section>
        ...
        <section id="faq" className={s.faq} aria-labelledby="faq-h">
          <div className={s.head}>
            ...
            <h2 id="faq-h">What parents ask us first</h2>
          </div>
      ...
      <footer className={s.footer}>
        <div className={s.footGrid}>
          <div>
            <p className={s.footName}>Pinecone Camp</p>
            <p className={s.footTag}>A summer day camp on the north shore of Loon Lake.</p>
          </div>
          <div>
            <h2 className={s.footHead}>Find us</h2>
            <p className={s.footText}>1 Big Pine Road, off Route 9</p>
            <p className={s.footText}>Gate opens 8:00 on camp days</p>
          </div>
          <div>
            <h2 className={s.footHead}>Talk to us</h2>
            <ul className={s.footLinks}>
              <li><a href="tel:5550148800">(555) 014-8800</a></li>
              <li><a href="mailto:hello@pineconecamp.example">hello@pineconecamp.example</a></li>
              <li><a href="#faq">Questions</a></li>
            </ul>
          </div>
        </div>
        <div className={s.footFine}>
          <p>A fictional summer camp. Weeks, fees and people are invented.</p>
          <p>
            <span>Patterns by </span>
            <a href="https://tabbied.com" rel="noopener">Tabbied</a>
            <span>, drawn live in the camp's own colors; the pictures follow the same palette.</span>
          </p>
        </div>
      </footer>
    </div>
