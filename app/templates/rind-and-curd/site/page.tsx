  ...
  return (
    <div className={s.page}>
      ...
      <header className={s.bar}>
        <a className={s.mark} href="#top">Rind &amp; Curd</a>
      ...
      <main id="top">
        ...
        <section className={s.cover} aria-labelledby="cover-h">
          <p className={s.issue}>
            <span>No. 14</span>
            <span>The counter quarterly</span>
            <span>Autumn</span>
          </p>
          <h1 id="cover-h" className={s.masthead}>Rind &amp; Curd</h1>
          <p className={s.dek}>Cheese shop and tasting counter, 31 Dairy Lane, Old Market</p>

          <div className={s.coverGrid}>
            <ol className={s.coverLines}>
              {COVER_LINES.map(([page, line]) => (
                <li key={page}>
        ...
        <section id="counter" className={s.spread} aria-labelledby="counter-h">
          <p className={s.running}>
            ...
            <span>Rind &amp; Curd, autumn</span>
          </p>
          <div className={s.pages}>
            <div className={s.left}>
              <h2 id="counter-h" className={s.headline}>The <em>big wheels</em> are ready</h2>
              <p className={s.standfirst}>
                A letter from the shop, on why autumn is the best time of year
                to stand at a cheese counter.
              </p>
              <blockquote className={s.pull}>
                <p>Ask for a taste. It is the whole point of a counter.</p>
              </blockquote>
            </div>
            <div className={s.right}>
              <div className={s.columns}>
                {LETTER.map((para, i) => (
                  <p key={i} className={i === 0 ? s.dropcap : undefined}>{para}</p>
                ))}
                <p className={s.signoff}>Margot Keller, cheesemonger and owner</p>
              </div>
            </div>
          </div>
          <p className={s.folio}>
        ...
        <section id="notes" className={s.spread} aria-labelledby="notes-h">
          <p className={s.running}>
            ...
            <span>On the counter this season</span>
          </p>
          <div className={s.pages}>
            <div className={s.left}>
              <div className={s.stick}>
                <h2 id="notes-h" className={s.headline}>Seven for the <em>cold months</em></h2>
                <article className={s.feature}>
                  ...
                  <p className={s.kicker}>Cheese of the season</p>
                  <h3 className={s.featureName}>{FEATURE.name}</h3>
                  <p className={s.featureNote}>{FEATURE.note}</p>
            ...
            <div className={s.right}>
              <ol className={s.notes}>
                {CHEESES.map((c) => (
                  <li key={c.name}>
                    <h3 className={s.noteName}>{c.name}</h3>
                    <p className={s.noteMaker}>{c.maker}</p>
                    ...
                    <p className={s.notePrice}>{c.price}</p>
                  </li>
                ))}
              </ol>
            </div>
        ...
        <section id="board" className={s.spread} aria-labelledby="board-h">
          <p className={s.running}>
            ...
            <span>A service piece</span>
          </p>
          <div className={s.pages}>
            <div className={s.left}>
              <h2 id="board-h" className={s.headline}>How much cheese, <em>really</em></h2>
              <p className={s.standfirst}>
            ...
            <div className={s.right}>
              <table className={s.amounts}>
                <caption className={s.tableCaption}>By the number of guests</caption>
                <thead>
                  <tr>
                    <th scope="col">Guests</th>
                    <th scope="col">Cheeses</th>
                    <th scope="col">As dinner</th>
                    <th scope="col">After dinner</th>
                    <th scope="col">Roughly</th>
                  </tr>
                </thead>
                <tbody>
                  {AMOUNTS.map(([guests, count, dinner, after, cost]) => (
                    <tr key={guests}>
        ...
        <div className={s.holes} aria-hidden="true">
          <TabbiedPattern
            pattern={dotfade}
            palette={HOLES}
            fit="grid"
        ...
        <section id="thursdays" className={s.spread} aria-labelledby="thursdays-h">
          <p className={s.running}>
            <span>Thursdays</span>
            <span>Tasting nights</span>
          </p>
          <div className={s.pages}>
            <div className={s.left}>
              <h2 id="thursdays-h" className={s.headline}>Thursday, <em>half past six</em></h2>
              <p className={s.standfirst}>
              ...
              <ol className={s.tastings}>
                {TASTINGS.map((t) => (
                  <li key={t.date} className={t.seats === 'Sold out' ? s.soldOut : undefined}>
                    <p className={s.tDate}>
                      <span className={s.tNum}>{t.date}</span>
                      <span className={s.tMon}>{t.day}</span>
                    </p>
                    <div>
                      <h3 className={s.tTheme}>{t.theme}</h3>
                      <p className={s.tWhat}>{t.what}</p>
                    </div>
                    <p className={s.tSeats}>{t.seats}</p>
                  </li>
                ))}
              </ol>
            </div>
        ...
        <section id="visit" className={s.spread} aria-labelledby="visit-h">
          ...
          <div className={s.pages}>
            <div className={s.left}>
              <h2 id="visit-h" className={s.headline}>31 Dairy Lane, <em>Old Market</em></h2>
              <p className={s.body}>
              ...
              <p className={s.contact}>
                <a href="tel:+15550126630">(555) 012-6630</a>
              </p>
              <p className={s.contact}>
                <a href="mailto:counter@rindandcurd.example">counter@rindandcurd.example</a>
              </p>
            </div>
            <div className={s.right}>
              ...
              <form className={s.form} action="#">
                <h3 className={s.formTitle}>Reserve a Thursday seat</h3>
                <div className={s.formGrid}>
                  ...
                  <div className={s.field}>
                    <label htmlFor="rc-night">Night</label>
                  ...
                  <div className={s.field}>
                    <label htmlFor="rc-seats">Seats</label>
                ...
                <button className={s.submit} type="submit">Hold my seats</button>
              </form>
            </div>
          </div>
          <p className={s.folio}>
      ...
      <footer className={s.footer}>
        <p className={s.footName}>Rind &amp; Curd</p>
        <p>A fictional cheese shop and tasting counter. The makers, cheeses, prices and dates are invented.</p>
        <p>
