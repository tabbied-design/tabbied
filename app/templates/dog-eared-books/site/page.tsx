    ...
    <div className={s.page}>
      ...
      <header className={s.masthead}>
        <div className={s.folio}>
          <span>No. 612</span>
          <span className={s.folioMid}>The weekly edition of an independent bookshop</span>
          <span>Free to browse</span>
        </div>
        <div className={s.titleRow}>
          <div className={s.ear}>
            <span className={s.earHead}>Open today</span>
            <span className={s.earBody}>9 am to 8 pm</span>
          </div>
          <a className={s.title} href="#top">Dog-Eared Books</a>
          <div className={s.ear}>
            <span className={s.earHead}>Special orders</span>
            <span className={s.earBody}>Any book in print, in 2-3 days</span>
          </div>
        </div>
        <div className={s.dateline}>
          <span>14 Linden Row, the Old Quarter</span>
          <span>Est. 2014</span>
          <span>(555) 016-4410</span>
        </div>
      ...
      <main id="top">
        ...
        <section id="picks" className={s.sec} aria-labelledby="picks-h">
          ...
          <div className={s.front}>
            <figure className={s.leadFig}>
              <div className={s.leadField} aria-hidden="true">
                <TabbiedPattern
              ...
              <figcaption>Fig. 1. The front table at ten on a Saturday morning, approximately.</figcaption>
            </figure>
            <article className={s.lead}>
              <span className={s.pickSection}>{LEAD.section}</span>
              <h3 className={s.leadTitle}>{LEAD.title}</h3>
              <p className={s.pickAuthor}>{LEAD.author}</p>
              <p className={s.leadReview}>{LEAD.review}</p>
              <p className={s.pickBy}>{LEAD.by}</p>
              <p className={s.pickPrice}>{LEAD.price}</p>
            </article>
          </div>
          ...
          <div className={s.columns}>
            {PICKS.map((p) => (
              <article key={p.title} className={s.pick}>
                <span className={s.pickSection}>{p.section}</span>
                <h3 className={s.pickTitle}>{p.title}</h3>
                <p className={s.pickAuthor}>{p.author}</p>
                <p className={s.pickReview}>{p.review}</p>
                <p className={s.pickBy}>{p.by}</p>
                <p className={s.pickPrice}>{p.price}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section id="events" className={s.sec} aria-labelledby="events-h">
          ...
          <h2 className={s.headline} id="events-h">Readings, signings and the book club</h2>
          <p className={s.standfirst}>
          ...
          <table className={s.events}>
            <caption className={s.visuallyHidden}>Events in October</caption>
            <thead>
              <tr>
              ...
              {EVENTS.map((e) => (
                <tr key={e.date}>
                  <td className={s.evDate}>
                    <span className={s.evDay}>{e.day}</span>
                    <span className={s.evNum}>{e.date}</span>
                  </td>
                  <td className={s.evTime}>{e.time}</td>
                  <td className={s.evWhat}>
                    <span className={s.evKind}>{e.kind}</span>
                    <strong className={s.evTitle}>{e.title}</strong>
                    <span className={s.evNote}>{e.note}</span>
                  </td>
                  <td className={s.evTickets}>{e.tickets}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <section id="new" className={s.sec} aria-labelledby="new-h">
          ...
          <div className={s.newGrid}>
            <div className={s.newIntro}>
              <h2 className={s.headline} id="new-h">Ten new arrivals, unpacked on Tuesday</h2>
              <p className={s.standfirst}>
            ...
            <ol className={s.newList}>
              {NEW_BOOKS.map(([title, author, format]) => (
                <li key={title}>
                  <span className={s.newTitle}>{title}</span>
                  <span className={s.newAuthor}>{author}</span>
                  <span className={s.newFormat}>{format}</span>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section id="orders" className={s.sec} aria-labelledby="orders-h">
          <div className={s.secRule}>
            <span>Orders and requests</span>
            <span>Page 4</span>
          </div>
          <div className={s.ordersGrid}>
            <div className={s.ordersText}>
              <h2 className={s.headline} id="orders-h">If we do not have it, we will get it</h2>
            ...
            <div className={s.ad}>
              <div className={s.adField} aria-hidden="true">
                <TabbiedPattern
                  pattern={halftone}
                  palette={DOTS}
                  fit="grid"
              ...
              <div className={s.adBody}>
                <p className={s.adHead}>Order any book in print</p>
                <p className={s.adSub}>Held at the counter, or posted for $4.50</p>
                <form className={s.form} action="#">
                  ...
                  <button className={s.btn} type="submit">Place the order</button>
                </form>
              </div>
            </div>
          </div>
        ...
        <section id="shelves" className={s.sec} aria-labelledby="shelves-h">
          ...
          <div className={s.shelvesGrid}>
            <figure className={s.shelfFig}>
              <div className={s.shelfField} aria-hidden="true">
                <TabbiedPattern
                  pattern={moleskin}
                  palette={SPINES}
                  fit="grid"
              ...
              <figcaption>Fig. 2. Fiction, A to K, seen from the top of the ladder.</figcaption>
            </figure>
            <div>
              <h2 className={s.headline} id="shelves-h">Where everything lives</h2>
              <p className={s.standfirst}>
              ...
              <dl className={s.shelves}>
                {SHELVES.map(([name, where, note]) => (
                  <div key={name}>
                    <dt>{name}</dt>
                    <dd className={s.shelfWhere}>{where}</dd>
                    <dd className={s.shelfNote}>{note}</dd>
                  </div>
                ))}
              </dl>
            </div>
        ...
        <section id="visit" className={s.sec} aria-labelledby="visit-h">
          ...
          <h2 className={s.headline} id="visit-h">Hours, address and how to reach us</h2>
          <div className={s.visit}>
            <div className={s.visitBox}>
              <h3 className={s.boxHead}>Opening hours</h3>
              ...
              <p className={s.boxNote}>Open until ten on event nights. Closed on public holidays.</p>
            </div>
            <div className={s.visitBox}>
              <h3 className={s.boxHead}>Find us</h3>
              <p className={s.visitAddr}>14 Linden Row, the Old Quarter</p>
            ...
            <div className={s.visitBox}>
              <h3 className={s.boxHead}>Get in touch</h3>
              <ul className={s.contact}>
                <li>
                  <a href="tel:+15550164410">(555) 016-4410</a>
                </li>
                <li>
                  <a href="mailto:shop@dogearedbooks.example">shop@dogearedbooks.example</a>
                </li>
              </ul>
              <p className={s.boxNote}>
      ...
      <footer className={s.footer}>
        <p className={s.footTitle}>Dog-Eared Books</p>
        <div className={s.footFine}>
          <p>A fictional bookshop. Titles, authors, events and prices are invented.</p>
          <p>
