 ...
 *
 * With no BETTER_AUTH_SECRET the answer is "nobody", not a lookup:
 * better-auth falls back to a well-known default secret outside
 * NODE_ENV=production (which workerd never sets), and with the cookie cache on
 * a cookie signed with it would be accepted with no database read. Dev keeps
 * the default so `.dev.vars` need not carry a secret.
 */
